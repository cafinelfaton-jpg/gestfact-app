...
export default function Articles() {
  ...
  async function sauvegarder() {
    ...
    if (!form.prix_unitaire) { showAlerte('Le prix est obligatoire'); return }
    setLoading(true)
  ...
  async function supprimer(id) {
    const ok = await showConfirm('Supprimer cet article ?')
    if (!ok) return
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <input className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Nom de l'article *" value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })} />
          <div className="flex gap-2">
            <input type="number" className="border border-gray-300 rounded-lg px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Prix unitaire *" value={form.prix_unitaire} onChange={e => setForm({ ...form, prix_unitaire: e.target.value })} />
            <select className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.unite} onChange={e => setForm({ ...form, unite: e.target.value })}>
              <option>pièce</option>
          ...
          <input className="border border-gray-300 rounded-lg px-3 py-2 text-sm sm:col-span-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Description (optionnel)" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
        </div>
